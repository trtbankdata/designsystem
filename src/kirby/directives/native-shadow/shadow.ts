...
function getAndroidR(rtype: string, field: string): number {
  ...
  if (!classCache[className].fieldCache.hasOwnProperty(field)) {
    classCache[className].fieldCache[field] = +classCache[className].class
      .getField(field)
      .get(null);
  }
...
export class Shadow {
  ...
  static apply(tnsView: any, data: IOSShadow | AndroidShadow) {
    const LOLLIPOP = 21;
  ...
  private static getDefaults(data: IOSShadow | AndroidShadow) {
    return Object.assign({}, data, {
      shape: (data as AndroidShadow).shape || Shadow.DEFAULT_SHAPE,
      pressedElevation: (data as AndroidShadow).pressedElevation || Shadow.DEFAULT_PRESSED_ELEVATION,
      pressedTranslationZ:
        (data as AndroidShadow).pressedTranslationZ || Shadow.DEFAULT_PRESSED_ELEVATION,
      shadowColor: (data as IOSShadow).shadowColor || Shadow.DEFAULT_SHADOW_COLOR,
      useShadowPath:
        (data as IOSShadow).useShadowPath !== undefined ? (data as IOSShadow).useShadowPath : true,
      rasterize: (data as IOSShadow).rasterize !== undefined ? (data as IOSShadow).rasterize : false,
    });
  }
  ...
  private static applyOnAndroid(tnsView: any, data: AndroidShadow) {
  ...
  private static overrideDefaultAnimator(nativeView: any, data: AndroidShadow) {
    ...
    notPressedSet.playTogether(
      java.util.Arrays.asList([
        ObjectAnimator.ofFloat(nativeView, 'translationZ', [z]).setDuration(buttonDuration),
  ...
  private static applyOnIOS(tnsView: any, data: IOSShadow) {
    ...
    nativeView.layer.shadowOffset = data.shadowOffset
      ? CGSizeMake(0, parseFloat(String(data.shadowOffset)))
      : CGSizeMake(0, 0.54 * elevation - 0.14);
    nativeView.layer.shadowOpacity = data.shadowOpacity
      ? parseFloat(String(data.shadowOpacity))
      : 0.006 * elevation + 0.25;
    nativeView.layer.shadowRadius = data.shadowRadius
    ...
    if (data.useShadowPath) {
      shadowPath = UIBezierPath.bezierPathWithRoundedRectCornerRadius(
        nativeView.bounds,
